...
export default function ContractUI({ walletAddress, onConnect, isConnecting }: ContractUIProps) {
  ...
  const checkVoterStatus = useCallback(async () => {
    ...
    try {
      const voted = await hasVoted(walletAddress) as boolean;
  ...
  }, [activeTab, loadResults, checkVoterStatus]);
  ...
  const handleVote = useCallback(async () => {
    ...
    try {
      await vote(walletAddress, candidateName.trim());
  ...
  }, [walletAddress, candidateName, loadResults]);
          ...
          <div className="p-6">
            ...
            {activeTab === "vote" && (
              <div className="space-y-5">
                ...
                {userHasVoted && (
                  <div className="rounded-xl border border-[#fbbf24]/15 bg-[#fbbf24]/[0.05] px-4 py-3 flex items-center gap-3">
                    <span className="text-[#fbbf24]"><CheckIcon /></span>
                    <span className="text-sm text-[#fbbf24]/90">You have already voted</span>
                  </div>
                )}

                {/* Popular candidates quick select */}
                {candidates.length > 0 && (
                  <div className="space-y-2">
                    <label className="block text-[11px] font-medium uppercase tracking-wider text-white/30">Quick Select</label>
                ...
                <div className="space-y-2">
                  <label className="block text-[11px] font-medium uppercase tracking-wider text-white/30">
                    {candidates.length > 0 ? "Or Enter New Candidate" : "Candidate Name"}
                  </label>
                  <div className="group rounded-xl border border-white/[0.06] bg-white/[0.02] p-px transition-all focus-within:border-[#34d399]/30 focus-within:shadow-[0_0_20px_rgba(52,211,153,0.08)]">
                    <input
                      ...
                      placeholder="Type any candidate name..."
                ...
                {walletAddress ? (
                  userHasVoted ? (
                    <button
                      disabled
                      className="w-full rounded-xl border border-[#fbbf24]/20 bg-[#fbbf24]/[0.03] py-4 text-sm text-[#fbbf24]/60"
                    >
                      Already voted — cannot vote twice
                    </button>
                  ) : (
                    <ShimmerButton onClick={handleVote} disabled={isVoting || !candidateName.trim()} shimmerColor="#34d399" className="w-full">
              ...
              <div className="space-y-5">
                ...
                {isLoadingResults ? (
                  ...
                  <div className="rounded-xl border border-dashed border-white/[0.08] bg-white/[0.02] p-8 text-center">
                    <p className="text-sm text-white/40">No votes yet.</p>
                    <p className="text-xs text-white/25 mt-1">Be the first to vote!</p>
                  </div>
                ) : (
